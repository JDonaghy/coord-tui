...
//!
//! **#13, first slice:** the blocking-modal keyboard intercepts that used to be
//! ~1,000 inlined lines of `dispatch_handle` now live in
//! [`super::events_modals`] as `-> Option<Reaction>` early-return helpers.
//! `dispatch_handle` calls them in the original block order, which is
//! load-bearing when two modals are alive at once — read that module's header
//! before reordering. New modal key handling belongs there, not here; that is
//! the point of the split (this function was the crate's #1 merge-conflict
//! site). The per-view arms of the big `match &event` below are the unfinished
//! remainder of #13.
//!
...
impl CoordApp {
    ...
    pub(crate) fn dispatch_handle(
        ...
        // ── Post-stage one-key offers (#13) ──────────────────────────────
        // Auto-review, next-stage, rework, test→fix, test→merge and
        // force-past-cap confirms, hoisted into `app/events_modals.rs`.
        // Deliberately BEFORE the detail-terminal focus block below: a
        // still-focused shell would otherwise eat the Enter.
        if let Some(reaction) = self.handle_stage_offer_keys(&event) {
            return reaction;
        }
        ...
        // ── Blocking modal tier, part 1/5 (#13) ────────────────────────────
        // The modal key intercepts live in `app/events_modals.rs`. Each helper
        // returns `Some(reaction)` when it claimed the key and `None` to fall
        // through. THE CALL ORDER HERE IS THE ORIGINAL BLOCK ORDER AND IS
        // LOAD-BEARING — see that module's header before reordering.
        if let Some(reaction) = self.handle_choice_picker_keys(&event) {
            return reaction;
        }
        // ── Blocking modal tier, part 2a/5 (#13): single-field text prompts ─
        if let Some(reaction) = self.handle_text_prompt_keys(&event) {
            return reaction;
        }
        // ── Blocking modal tier, part 2b/5 (#13): destructive confirms ─────
        if let Some(reaction) = self.handle_destructive_confirm_keys(&event) {
            return reaction;
        }
        // ── Blocking modal tier, part 3/5 (#13): open context menu ─────────
        if let Some(reaction) = self.handle_context_menu_keys(&event, backend) {
            return reaction;
        }
        // ── Blocking modal tier, part 4/5 (#13): merge-path confirmations ──
        if let Some(reaction) = self.handle_merge_confirm_keys(&event) {
            return reaction;
        }
        // ── Blocking modal tier, part 5/5 (#13): read-only info dialogs ──
        // PTY-panic (#816) → Gate-A / decompose failure (#1059/#2863) →
        // artifact-pull (#532), in that order because that is the order
        // `build_prompt_dialog` stacks them: the dialog drawn on top is the one
        // that must receive the keystroke.
        //
        // #13 note: the PTY-panic and Gate-A blocks each appeared TWICE in the
        // inlined original — once here and once again immediately after the
        // artifact-pull block. The second copies were unreachable (the first
        // pair returns for every `KeyPressed` whenever their guard holds), so
        // they were dropped rather than carried into `events_modals.rs`.
        if let Some(reaction) = self.handle_info_dialog_keys(&event, backend) {
            return reaction;
        }
